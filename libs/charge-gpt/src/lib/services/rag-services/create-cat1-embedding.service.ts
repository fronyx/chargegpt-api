import { cat1FewShotExamples } from './cat1-few-shots.model';
...
export const createCat1Embeddings = async (): Promise<void> => {
  console.log('Creating Category 1 embeddings...');
  const fewShotRequests = cat1FewShotExamples.map((example) =>
  ...
  const fewShotExamples = cat1FewShotExamples.map((example) =>
  ...
  disconnectRagClient();
  console.log('Category 1 embeddings created.');
};

export const retrieveCat1Embeddings = async (
  userRequest: string,
