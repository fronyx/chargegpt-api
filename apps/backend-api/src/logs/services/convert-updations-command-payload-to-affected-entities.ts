...
export function convertUpdationsCommandPayloadToAffectedEntities(args: { payloads: OcpiActionsCommandPayload[]; }): OcpiCommandPayloadConversions {
  const connectors: OcpiConnector[] = args.payloads
    .filter(filterByConnectorParams)
    .map(({ params, data }) => OcpiConnector.createFromUpdatePayload({
      ...data,
      ...params,
  ...
  const evses: OcpiEvse[] = [].concat(
    ...
    args.payloads
      ...
      .map(({ params, data }) => OcpiEvse.createFromUpdatePayload({
        ...data,
        ...params,
      })),
  );
  ...
  const locations: OcpiLocation[] = [].concat(
    ...
    args.payloads
      ...
      .map(({ params, data }) => OcpiLocation.createFromUpdatePayload({
        ...data,
        ...params,
      })),
  );
